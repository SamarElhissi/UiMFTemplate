namespace UiMFTemplate.Core.Notification
{
	...
	public class MyNotifications : MyForm<MyNotifications.Request, MyNotifications.Response>
	{
		...
		{
			[Description("Show only unArchive")] ShowNotArchivedOnly = 1,
			[Description("Show only archive")] ShowArchivedOnly = 2
		}
			...
			return new Response
			{
				...
				Items = notifications.Transform(t => new NotificationRow
				{
					...
					Actions = GetActions(t,
						this.notificationManagerRegister.GetInstance(t.RelatedTo.EntityType).GetLink(t.RelatedTo.EntityId).Actions)
				})
			};
		}
